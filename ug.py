...
def get_mps_and_keys(api_url): #ऑटो टोकन वाली api के 
...
async def decrypt_and_merge_video(mpd_url, keys_string, output_path=None, output_name=None, quality="720"):
    try:
        # 🔹 Default path and name if missing
        if not output_path:
            output_path = "./downloads"  # current folder में 'downloads' नाम का folder
        if not output_name:
            output_name = f"video_{int(time.time())}"  # timestamp-based नाम
